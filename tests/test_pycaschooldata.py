"""
Tests for pycaschooldata Python wrapper.
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pycaschooldata as ca
        assert hasattr(ca, 'fetch_enr')
        assert hasattr(ca, 'fetch_enr_multi')
        assert hasattr(ca, 'tidy_enr')
        assert hasattr(ca, 'get_available_years')
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        import pycaschooldata as ca
        max_year = get_test_years()['max_year']
        df = ca.fetch_enr_multi([max_year - 1, max_year])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_more_rows_than_single_year(self):
        ...
        import pycaschooldata as ca
        max_year = get_test_years()['max_year']
        df_single = ca.fetch_enr(max_year)
        df_multi = ca.fetch_enr_multi([max_year - 1, max_year])
...
class TestTidyEnr:
    ...
    @pytest.mark.skip(reason="tidy_enr has compatibility issues with current rpy2 version")
    def test_returns_dataframe(self):
        """Returns a pandas DataFrame."""
        import pycaschooldata as ca
        max_year = get_test_years()['max_year']
...
class TestDataIntegrity:
    ...
    def test_consistent_between_single_and_multi(self):
        ...
        max_year = get_test_years()['max_year']
        df_single = ca.fetch_enr(max_year)
        df_multi = ca.fetch_enr_multi([max_year])
    ...
    def test_years_within_available_range(self):
        ...
        df = ca.fetch_enr(years['max_year'])
        assert len(df) > 0
