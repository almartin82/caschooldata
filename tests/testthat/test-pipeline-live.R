# ==============================================================================
# LIVE Pipeline Tests for caschooldata
# ==============================================================================
#
...
test_that("California DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add California DOE URL and verify HTTP 200")
})
...
test_that("Can download California enrollment data file", {
  skip_if_offline()
...
test_that("Can parse California enrollment file", {
  skip_if_offline()
...
test_that("caschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- caschooldata::get_available_years()
  ...
  tryCatch({
    raw <- caschooldata:::get_raw_enr(test_year)
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- caschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- caschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- caschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- caschooldata:::get_cache_path(2024, "enrollment")
