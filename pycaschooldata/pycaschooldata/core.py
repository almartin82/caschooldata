"""
Core functions wrapping caschooldata R package via rpy2.
"""
...
def _get_pkg():
    ...
    global _pkg
    if _pkg is None:
        _pkg = importr("caschooldata")
    return _pkg


def fetch_enr(end_year: int) -> pd.DataFrame:
    """
    Fetch California school enrollment data for a single year.
...
def fetch_enr_multi(end_years: list[int]) -> pd.DataFrame:
    """
    Fetch California school enrollment data for multiple years.
    ...
    --------
    >>> import pycaschooldata as ca
    >>> df = ca.fetch_enr_multi([2020, 2021, 2022])
    """
    pkg = _get_pkg()
    with localconverter(robjects.default_converter + pandas2ri.converter):
...
def get_available_years() -> dict:
    ...
    with localconverter(robjects.default_converter + pandas2ri.converter):
        ...
        # Handle various rpy2 return types (dict, NamedList, ListVector, vector)
        ...
        # CA returns a vector of years like c(1982, 1983, ..., 2025)
